// Register.jsx, Allows new users to create an account.
// Performs client‑side validation (name, email format, password length) before calling the backend.
...
// Shared TextField styles, matches the input styling used in Login.jsx.
const inputSx = {
  ...
  return (
    ...
    >
      <Container maxWidth="sm" sx={{ position: "relative", zIndex: 1 }}>
        ...
        {/* White card with a subtle fade‑up animation */}
        <Box
          sx={{
        ...
        >
          ...
          {error && (
            <Alert severity="error" sx={{ mb: 3, borderRadius: 2, fontFamily: "inherit" }}>
              {error}
            </Alert>
          )}
          {success && (
            <Alert severity="success" sx={{ mb: 3, borderRadius: 2, fontFamily: "inherit" }}>
              {success}
            </Alert>
          )}
          ...
          <Box component="form" onSubmit={handleSubmit}>
            {[
              ...
              <Box key={name}>
                {/* Each field gets an uppercase label above the input, consistent with Login and JoinGroup. */}
                <Typography
                  sx={{
                    fontFamily: "inherit",
                    fontWeight: 700,
